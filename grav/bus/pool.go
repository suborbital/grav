...
func (c *connectionPool) insert(pod *pod.Pod) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
type podConnection struct {
	ID          int64
	messageChan message.MsgChan
	errorChan   message.MsgChan
...
func newPodConnection(id int64, pod *pod.Pod) *podConnection {
...
func (p *podConnection) emit(msg message.Message) {
	go func() {
		p.messageChan <- msg
	}()
}

// checkErr checks the pod's errChan for any failed messages and drains the channel into the failed message.Message buffer
